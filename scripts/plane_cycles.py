...
def cmd_list(args: argparse.Namespace) -> None:
    ...
    response = client.cycles.list(slug, args.project_id)
...
def cmd_create(args: argparse.Namespace) -> None:
    ...
    cycle = client.cycles.create(slug, args.project_id, payload)
    print(dump_json(cycle.model_dump()))


def cmd_get(args: argparse.Namespace) -> None:
    client, slug = get_client()
    cycle = client.cycles.retrieve(slug, args.project_id, args.cycle_id)
    print(dump_json(cycle.model_dump()))
...
def cmd_update(args: argparse.Namespace) -> None:
    ...
    cycle = client.cycles.update(slug, args.project_id, args.cycle_id, payload)
    print(dump_json(cycle.model_dump()))
...
def cmd_delete(args: argparse.Namespace) -> None:
    if not args.confirm:
        ...
        sys.exit(1)
    client, slug = get_client()
    client.cycles.delete(slug, args.project_id, args.cycle_id)
    print(dump_json({"status": "deleted", "cycle_id": args.cycle_id}))


def cmd_archive(args: argparse.Namespace) -> None:
    client, slug = get_client()
    client.cycles.archive(slug, args.project_id, args.cycle_id)
    print(dump_json({"status": "archived", "cycle_id": args.cycle_id}))


def cmd_unarchive(args: argparse.Namespace) -> None:
    client, slug = get_client()
    client.cycles.unarchive(slug, args.project_id, args.cycle_id)
...
def cmd_add_items(args: argparse.Namespace) -> None:
    ...
    client.cycles.add_work_items(slug, args.project_id, args.cycle_id, issue_ids)
    print(dump_json({"status": "added", "cycle_id": args.cycle_id, "issue_ids": issue_ids}))


def cmd_remove_item(args: argparse.Namespace) -> None:
    client, slug = get_client()
    client.cycles.remove_work_item(slug, args.project_id, args.cycle_id, args.work_item_id)
...
def cmd_list_items(args: argparse.Namespace) -> None:
    client, slug = get_client()
    response = client.cycles.list_work_items(slug, args.project_id, args.cycle_id)
    results = response.results if hasattr(response, "results") else response
    data = [r.model_dump() if hasattr(r, "model_dump") else r for r in results]
    print(dump_json(data))
...
def cmd_transfer_items(args: argparse.Namespace) -> None:
    ...
    client.cycles.transfer_work_items(slug, args.project_id, args.cycle_id, payload)
    print(dump_json({
        "status": "transferred",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_transfer.add_argument("--project-id", required=True, help="Project UUID")
