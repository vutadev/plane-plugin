...
def cmd_list(args: argparse.Namespace) -> None:
    ...
    response = client.states.list(slug, args.project_id)
...
def cmd_create(args: argparse.Namespace) -> None:
    ...
    state = client.states.create(slug, args.project_id, payload)
    print(dump_json(state.model_dump()))


def cmd_get(args: argparse.Namespace) -> None:
    client, slug = get_client()
    state = client.states.retrieve(slug, args.project_id, args.state_id)
    print(dump_json(state.model_dump()))
...
def cmd_update(args: argparse.Namespace) -> None:
    ...
    state = client.states.update(slug, args.project_id, args.state_id, payload)
    print(dump_json(state.model_dump()))
...
def cmd_delete(args: argparse.Namespace) -> None:
    if not args.confirm:
        ...
        sys.exit(1)
    client, slug = get_client()
    client.states.delete(slug, args.project_id, args.state_id)
