...
def cmd_list(args: argparse.Namespace) -> None:
    ...
    response = client.intake.list(slug, args.project_id)
...
def cmd_create(args: argparse.Namespace) -> None:
    ...
    item = client.intake.create(slug, args.project_id, payload)
    print(dump_json(item.model_dump()))


def cmd_get(args: argparse.Namespace) -> None:
    client, slug = get_client()
    item = client.intake.retrieve(slug, args.project_id, args.work_item_id)
    print(dump_json(item.model_dump()))
...
def cmd_update(args: argparse.Namespace) -> None:
    ...
    item = client.intake.update(slug, args.project_id, args.work_item_id, payload)
    print(dump_json(item.model_dump()))
...
def cmd_delete(args: argparse.Namespace) -> None:
    if not args.confirm:
        ...
        sys.exit(1)
    client, slug = get_client()
    client.intake.delete(slug, args.project_id, args.work_item_id)
