...
def cmd_get_project(args: argparse.Namespace) -> None:
    client, slug = get_client()
    page = client.pages.retrieve_project_page(slug, args.project_id, args.page_id)
    print(dump_json(page.model_dump()))
...
def cmd_create_project(args: argparse.Namespace) -> None:
    ...
    payload = CreatePage(**fields)
    page = client.pages.create_project_page(slug, args.project_id, payload)
    print(dump_json(page.model_dump()))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_gp.add_argument("--project-id", required=True, help="Project UUID")
    ...
    p_cp.add_argument("--project-id", required=True, help="Project UUID")
